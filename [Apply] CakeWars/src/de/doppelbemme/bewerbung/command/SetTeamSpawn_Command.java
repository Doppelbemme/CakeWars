...
public class SetTeamSpawn_Command implements CommandExecutor {
		...
		if(!(sender instanceof Player)){
			...
			if(!player.hasPermission("cakewars.setup")){
				...
				if(teamNumber.equalsIgnoreCase("1")){
					Bewerbung.main.fm.setSpawnTeam1(player.getLocation(), mapname);
					player.sendMessage(Bewerbung.main.prefix + "§aDer §eTeam Spawn 1 §aauf der Map §e" + mapname + " §awurde erfolgreich gesetzt.");
					player.playSound(PlayerLoc, Sound.ORB_PICKUP, 1.0F, 1.0F);
				}else if(teamNumber.equalsIgnoreCase("2")){
					Bewerbung.main.fm.setSpawnTeam2(player.getLocation(), mapname);
					player.sendMessage(Bewerbung.main.prefix + "§aDer §eTeam Spawn 2 §aauf der Map §e" + mapname + " §awurde erfolgreich gesetzt.");
					player.playSound(PlayerLoc, Sound.ORB_PICKUP, 1.0F, 1.0F);
				}else{
					Bewerbung.sendSetupMessage(player);
				}
